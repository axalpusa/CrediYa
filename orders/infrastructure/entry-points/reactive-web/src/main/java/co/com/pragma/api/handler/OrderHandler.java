...
public class OrderHandler {
    ...
    private Mono < AuthResponseDTO > validateUserToken(ServerRequest request, UUID idRol) {
        ...
        if ( authHeader == null || !authHeader.startsWith ( "Bearer " ) ) {
            ...
            return Mono.error ( new RuntimeException ( Constants.USER_INVALID ) );
        }
        ...
        return authServiceClient.validateToken ( token )
                .flatMap ( user -> {
                    ...
                    if ( !allowed ) {
                        return Mono.error ( new RuntimeException ( Constants.USER_NOT_ALLOWED ) );
                    }
                        ...
                        .flatMap ( authUser ->
                                request.bodyToMono ( OrderResponseDTO.class )
                                        .flatMap ( dto -> orderUseCase.getOrderById ( dto.getIdOrder ( ) )
                                                .switchIfEmpty ( Mono.error ( new NotFoundPragmaException ( Constants.ORDER_NOT_FOUND ) ) )
                                                .flatMap ( existing -> {
